...
This file calls runme_pick_v2.py to run neuron model simulation and calculate spike triggered average (STA) for OU stimulus. Here we only keeps spikes that don't have any spikes before them in certain amount of time for STA.
...
if model == 'GE_diam_0_vshift0':
  v_loc = -74.25
if model == 'GE_diam_3_vshift0':
  v_loc = -68.2
if model == 'GE_diam_5_vshift0':
  v_loc = -65.6
if model == 'GE_diam_0':
  v_loc = -58.05 
if model == 'GE_diam_3':
    ...
    for fr in (519,): 
      for amplitude in ([0],):
        for frequency in ([0],): # sinusoidal: ([1],[2],[5],[10],[20],[50],[100],[200],[500],[1000])
          ...
          param['spikedistance'] = 50 # ms the minimum distance of two spikes for STA
          param['v_loc'] = v_loc
          if frequency == [0] and amplitude == [0]: # 'OU'
            appendix = 'tau%sfr%sposNa%s_spikedistance%s_v2'%(tau, fr, posNa, param['spikedistance'])
          ...
          submit_job_array(runs, ".", "runme.sh", [model, "runme_pick_v2.py", foldername])
