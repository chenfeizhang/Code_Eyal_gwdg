...
for k in range(rep): 
  ...
  if stim_type == 'OU':
    ...
    sp_STA_pick = [sp_STA[0],]
    sp_ref = sp_STA[0] # reference spike time
    for j in range(len(sp_STA)):
      if sp_STA[j] - sp_ref < spikedistance/1000.0:
        sp_ref = sp_STA[j]
        continue
      else:
        sp_ref = sp_STA[j]
        sp_STA_pick.append(sp_STA[j])
    if len(sp_STA_pick) != 0:
      print(sp_STA_pick)
      ...
      for sp in sp_STA_pick:
        STA_tmp_add =  stim[(int(sp/dt_s)-maxtau-1):(int(sp/dt_s)+maxtau-1)] # spike triggered stimulus
        STA_tmp = STA_tmp + np.array(STA_tmp_add)  
    spiketimelist.append(sp_STA_pick)
    nspikes = nspikes + len(sp_STA_pick)
    b = np.diff((np.array(va[int(T_relax/dt):])>v_loc)*1.0)
    ...
    for sp in sp_STA_pick:
      if sum(np.diff((v_loc_id_all<int(sp/dt_s))*1.0)) == 0: # if sp is larger than all the v_loc_id, then the last one is paired.
        v_loc_pair.append(v_loc_id_all[-1])
      else:
        c = np.diff((v_loc_id_all<int(sp/dt_s))*1.0)
        v_loc_last_id = np.where(c==-1)[0][-1]
        v_loc_pair.append(v_loc_id_all[v_loc_last_id])
    spike_time_duration += list(np.array(sp_STA_pick)*1000 - np.array(v_loc_pair)*dt)
  sys.stdout.flush()
...
if stim_type == 'OU':
  if nspikes != 0:
    ...
    np.save(foldername + '/Series' + str(i) + '/spike_detection_duration', {'sdd':spike_time_duration})
